...
$optionsAll.on("change", function(){
    ...
    $(".options-list .checkbox-wrap img").attr("src", "/static/images/" + ($(this).prop("checked") ? "checked.png" : "check.png"));
});

$options.on("change", function(){
    $(".options-title input[type='checkbox']").prop("checked", $options.filter(":checked").length);
    $(".options-title .checkbox-wrap img").attr("src", "/static/images/" + ($(".options-title input[type='checkbox']").prop("checked") ? "checked.png" : "check.png"));
});
...
function checked($img){
    $img.attr("src", "/static/images/checked.png");
    checkAvaliable();
}

function notChecked($img){
    $img.attr("src", "/static/images/check.png");
    checkAvaliable();
}
...
function checkAvaliable(){
    ...
    $(".options input[type='checkbox']").next().find("img").attr("src", "/static/images/check.png");
}
